...
<head>
  <meta charset="UTF-8">
  <title>XSalsa20-Poly1305 (TweetNaCl) Text Encryptor</title>
...
<body>
  <h1>XSalsa20-Poly1305 (TweetNaCl) Text Encryptor/Decryptor</h1>

  <div class="row">
    <span class="label">Input (Plaintext or Ciphertext, Base64URL)</span>
  ...
  <div class="cols">
    <div class="row">
      <span class="label">Password (recommended ≥32 characters, high entropy)</span>
    ...
    <div class="row">
      ...
      <div class="hint">The iteration count is stored in the ciphertext header; the decryptor enforces a policy range to prevent malicious high-iteration DoS.</div>
    </div>
  </div>
  ...
  function parseHeader(u8){
    if(u8.length < HEADER_LEN + 16) throw new Error('Ciphertext too short');
    ...
    return {iter, salt, nonce, body};
  }

  async function deriveKeyRaw(passwordUtf8, salt, iterations){
  ...
  (function envCheck(){
    if(!crypto || !crypto.getRandomValues){
      alert('Missing secure random number generator (crypto.getRandomValues). Please use a modern browser.');
    }
    if(!crypto.subtle){
      alert('Missing WebCrypto SubtleCrypto. Open this page via http://localhost or use a browser with WebCrypto support.');
    }
  })();
  ...
  $encBtn.onclick = async () => {
    try{
      ...
      const key = await deriveKeyRaw(enc.encode(pwd), salt, iters);
      if(key.length !== 32) throw new Error('KDF output length error');

      const msg = enc.encode(text);
      const box = nacl.secretbox(msg, nonce, key);

      const out = concatBytes(buildHeader(iters, salt, nonce), box);
      const b64u = b64uEncode(out);
      $io.value = b64u;
      setStatus('Encryption complete', true);

      salt.fill(0); nonce.fill(0); key.fill(0);
    }catch(e){
  ...
  $decBtn.onclick = async () => {
    try{
      ...
      const {iter, salt, nonce, body} = parseHeader(blob);

      if(iter < MIN_ITERS || iter > MAX_ITERS) throw new Error('Iterations out of policy');

      const key = await deriveKeyRaw(enc.encode(pwd), salt, iter);
      if(key.length !== 32) throw new Error('KDF output length error');

      const msg = nacl.secretbox.open(body, nonce, key);
      if(!msg) throw new Error('Decryption failed: wrong password or data corrupted');
