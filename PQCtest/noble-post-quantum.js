var noblePostQuantum = (() => {
  ...
  function validateVerOpts(opts2) {
    validateOpts(opts2);
    if (opts2.context !== void 0) abytes(opts2.context, void 0, "opts.context");
  }
  ...
  var genCrystals = (opts2) => {
    ...
    const NTT3 = {
      ...
      decode: (r) => {
        dit(r);
        for (let i = 0; i < r.length; i++) r[i] = mod3(F3 * r[i]);
        return r;
      }
    };
  ...
  function validateInternalOpts(opts2) {
    validateOpts(opts2);
    if (opts2.externalMu !== void 0) abool(opts2.externalMu, "opts.externalMu");
  }
  var N = 256;
  var Q = 8380417;
  ...
  function getDilithium(opts2) {
    ...
    const internal = {
      ...
      keygen: (seed) => {
        ...
        if (randSeed) seed = randomBytes2(32);
        abytes(seed, 32, "seed");
        seedDst.set(seed);
        ...
        const s1 = [];
        for (let i = 0; i < L; i++) s1.push(RejBoundedPoly(xofPrime.get(i & 255, i >> 8 & 255)));
        const s2 = [];
        for (let i = L; i < L + K; i++)
  ...
  function polyAdd2(a, b) {
    for (let i = 0; i < N2; i++) a[i] = mod2(a[i] + b[i]);
  }
  ...
  var genKPKE = (opts2) => {
    ...
    return {
      ...
      keygen: (seed) => {
        ...
        const sHat = [];
        const tHat = [];
        for (let i = 0; i < K; i++) sHat.push(NTT2.encode(sampleCBD(PRF, sigma, i, ETA1)));
        const x = XOF(rho);
        for (let i = 0; i < K; i++) {
      ...
      encrypt: (publicKey, msg, seed) => {
        ...
        const rHat = [];
        for (let i = 0; i < K; i++) rHat.push(NTT2.encode(sampleCBD(PRF, seed, i, ETA1)));
        const x = XOF(rho);
      ...
      decrypt: (cipherText, privateKey) => {
        ...
        const tmp = new Uint16Array(N2);
        for (let i = 0; i < K; i++) polyAdd2(tmp, MultiplyNTTs2(sk[i], NTT2.encode(u[i])));
  ...
  function gen(opts2, hashOpts) {
    ...
    const setAddr = (opts3, addr = new Uint8Array(ADDR_BYTES)) => {
      ...
      if (height !== void 0) addr[OFFSET_CHAIN_ADDR] = height;
      if (layer !== void 0) addr[OFFSET_LAYER] = layer;
      if (type !== void 0) addr[OFFSET_TYPE] = type;
      if (chain !== void 0) addr[OFFSET_CHAIN_ADDR] = chain;
      if (hash !== void 0) addr[OFFSET_HASH_ADDR] = hash;
      if (index !== void 0) v.setUint32(OFFSET_TREE_INDEX, index, false);
      if (subtreeAddr) addr.set(subtreeAddr.subarray(0, OFFSET_TREE + 8));
      if (tree !== void 0) v.setBigUint64(OFFSET_TREE, tree, false);
      if (keypair !== void 0) {
    ...
    const treehash = (height, fn) => function treehash_i(context, leafIdx, idxOffset, treeAddr, info) {
      ...
      for (let idx = 0; ; idx++) {
        ...
        for (let i = idx, o = idxOffset, l = leafIdx; ; h++, i >>>= 1, l >>>= 1, o >>>= 1) {
          ...
          if ((i ^ l) === 1) authPath.subarray(h * N3).set(cur1);
    ...
    const wotsTreehash = treehash(TREE_HEIGHT, (leafIdx, addrOffset, context, info) => {
      ...
      for (let i = 0; i < WOTS_LEN; i++) {
        ...
        for (let k = 0; ; k++) {
          if (k === wotsK) info.wotsSig.subarray(i * N3).set(pk);
          if (k === W - 1) break;
    ...
    const internal = {
      ...
      keygen(seed) {
        if (seed !== void 0) abytes(seed, seedCoder.bytesLen, "seed");
      ...
      sign: (msg, sk, opts3 = {}) => {
        ...
        else if (random === void 0) random = randomBytes2(N3);
        ...
        for (let i = 0; i < indices.length; i++) {
          ...
          const { root: root2, authPath } = forsTreehash(
            context,
            indices[i],
            idxOffset,
            forsTreeAddr,
  ...
  var slh_dsa_shake_128s = /* @__PURE__ */ gen(PARAMS3["128s"], SHAKE_SIMPLE);
  var slh_dsa_shake_192f = /* @__PURE__ */ gen(PARAMS3["192f"], SHAKE_SIMPLE);
  var slh_dsa_shake_192s = /* @__PURE__ */ gen(PARAMS3["192s"], SHAKE_SIMPLE);
  var slh_dsa_shake_256f = /* @__PURE__ */ gen(PARAMS3["256f"], SHAKE_SIMPLE);
  var slh_dsa_shake_256s = /* @__PURE__ */ gen(PARAMS3["256s"], SHAKE_SIMPLE);
    ...
    return {
      ...
      Hmsg: (R, pk, m, outLen) => {
        ...
        const seed = concatBytes(
          R.subarray(0, N3),
          pk.subarray(0, N3),
          h1.create().update(R.subarray(0, N3)).update(pk).update(m).digest()
        );
  ...
  var slh_dsa_sha2_128s = /* @__PURE__ */ gen(PARAMS3["128s"], SHA256_SIMPLE);
  var slh_dsa_sha2_192f = /* @__PURE__ */ gen(PARAMS3["192f"], SHA512_SIMPLE);
  var slh_dsa_sha2_192s = /* @__PURE__ */ gen(PARAMS3["192s"], SHA512_SIMPLE);
  var slh_dsa_sha2_256f = /* @__PURE__ */ gen(PARAMS3["256f"], SHA512_SIMPLE);
  var slh_dsa_sha2_256s = /* @__PURE__ */ gen(PARAMS3["256s"], SHA512_SIMPLE);
